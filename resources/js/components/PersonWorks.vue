                ...
                <div class="grid gallery_work_wrapper is_auth" v-if="works.length > 0">
                    <div class="gallery_work grid-item"
                        ...
                        <div class="gallery_work__container">
                            ...
                            <div class="work_eye__icon work_icons person"
                                 @click="showGallery(workIndex)"></div>
                            <div class="work_like__container"
                                 @click="addDeleteLike(work.id,workIndex)">
                                <div class="work_like__container_relative">
                                    <div class="work_like__icon work_icons"
                                         :class="{blue:isLike(workIndex)}"></div>
                                    <div class="count_likes work_icons" v-if="work.number_of_likes > 0"
                                         :class="{blue:isLike(workIndex)}">{{ work.number_of_likes }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
