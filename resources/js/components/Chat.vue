                ...
                <div class="chat_content_left__persons" v-if="users.length > 0">
                    <div class="chat_content_left__person"  v-for="user in users" v-on:click="selectChatUser(user.id)" :class="{active:userSelect===user.id}">
                        ...
                        <div class="personal_chat_left_person__right_container" v-if="user.last_messages_from !== null">
                            <div class="personal_chat_left_person__date" v-if="user.last_messages_from.updated_at !== null">{{ user.last_messages_from.updated_at | moment("H : m")}}</div>
                            <div class="personal_chat_left_person__date" v-else>{{ user.last_messages_from.created_at | moment("H : m")}}</div>
                            <div class="count_message__chat" v-if="user.last_messages_from.count_of_unread > 0">{{ user.last_messages_from.count_of_unread}}</div>
                        </div>
                        <div class="personal_chat_left_person__right_container" v-if="user.last_messages_to !== null">
                            <div class="personal_chat_left_person__date" v-if="user.last_messages_to.updated_at !== null">{{ user.last_messages_to.updated_at | moment("H : m")}}</div>
                            <div class="personal_chat_left_person__date" v-else>{{ user.last_messages_to.updated_at | moment("H : m")}}</div>
                            <div class="count_message__chat" v-if="user.last_messages_to.count_of_unread > 0">{{ user.last_messages_to.count_of_unread}}</div>
                        </div>
                    </div>
                </div>
