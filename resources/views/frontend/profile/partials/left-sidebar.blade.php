            ...
            <div class="left_section_options_container">
                ...
                <div class="left-section__menu">
                    <ul class="left-menu__items">
                        <li>
                            <router-link class="left-menu__item-container left_menu_homepage"
                                         to="/profile/{{ $user->id }}">
                        ...
                        <li>
                            <a class="left-menu__item-container left_menu_friends" @click.stop="openFriendsContainer"
                               @click="clickMyFriends()" :class="{is_active:isOpenFriends}">
                        ...
                        <li>
                            <router-link @click="changeComponents" class="left-menu__item-container left_menu_mail"
                                         to="/profile/mail">
                                <span class="left-item__icon left_icon_mail"></span>
                                <p>Сообщения</p>
                                <span class="count_message" :class="{is_visible_flex:countOfUnreadMessages > 0}"
                                      v-html="countOfUnreadMessages"></span>
                            </router-link>
                        </li>
                        <li>
                        ...
                        <li>
                            <router-link @click="changeComponents" class="left-menu__item-container left_menu_contacts"
                                         to="/profile/settings">
                        ...
                        <li>
                            <router-link @click="changeComponents"
                                         class="left-menu__item-container left_menu_notifications"
                                         to="/profile/notifications">
                                <span class="left-item__icon left_icon_notifications"></span>
                                <p>Уведомления</p>
                                <span class="count_message" :class="{is_visible_flex:countOfNotifications > 0}"
                                      v-html="countOfNotifications"></span>
                            </router-link>
                        </li>
                    </ul>
                </div>
                <div class="left_section__friend_options" :class="{is_visible:isOpenFriends}" @click.stop="clickOut()"
                                                ...
                                                @if($friend->second_user == Auth::id())
                                                    ...
                                                    @if($friend->second_user == Auth::id())
                                                        <router-link to="/profile/{{ $friend->firstUser->id }}">
                                                            <div class="option_people_name"
                                                                 @click="changeComponentsToProfile({{ $friend->firstUser->id }})">
                                                    ...
                                                    @elseif($friend->first_user == Auth::id())
                                                        <router-link to="/profile/{{ $friend->secondUser->id }}">
                                                            <div class="option_people_name"
                                                                 @click="changeComponentsToProfile({{ $friend->secondUser->id }})">
                                                    ...
                                                    @if($friend->second_user == Auth::id())
                                                        <div class="option_people_dots"
                                                             @click.stop="clickOutFriendContainer()"
                                                             @click="selectFriend = {{ $friend->firstUser->id }}"
                                                             :class="{is_active:showFriendsOption({{ $friend->firstUser->id }})}">
                                                            <div class="option_people_dots__menu"
                                                                 v-show="showFriendsOption({{ $friend->firstUser->id }})">
                                                                @elseif($friend->first_user == Auth::id())
                                                                    <div class="option_people_dots"
                                                                         @click.stop="clickOutFriendContainer()"
                                                                         @click="selectFriend = {{ $friend->secondUser->id }}"
                                                                         :class="{is_active:showFriendsOption({{ $friend->secondUser->id }})}">
                                                                        <div class="option_people_dots__menu"
                                                                             v-show="showFriendsOption({{ $friend->secondUser->id }})">
                                                                            @endif
                                                                            <ul class="option_people_dots__ul">
                                                                                <li @click.stop="isOpenFriends = false">
                                                                                    Написать сообщение
                                                                                </li>
                                                                                <li @click.stop="isOpenFriends = false">
                                                                                    Посмотреть друзей
